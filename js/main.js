$(function(){
  ...
  $('.info-tabs__item').on('click', function (e){
    e.preventDefault();//ссылки перестали быть ссылками

    $('.info-tabs__item').removeClass('info-tabs__item--active'); //убирается подчеркивание когда нажимаем на другую ссылку
    $(this).addClass('info-tabs__item--active'); //добавляю туда куда кликну

    $(".info-tabs__content").removeClass('info-tabs__content--active');//убиарем при нажатии описание 
    $($(this).attr('href')).addClass('info-tabs__content--active'); //при нажатии по id показывается тот контетк который соединен с id
  });
